...
The system SHALL provide a TargetWorker that encapsulates the full reconciliation sequence for a single target: session acquisition, settings reconciliation, content reconciliation, gravity triggering, and metrics recording. The worker SHALL depend on interfaces, not concrete types.

#### Scenario: Normal reconcile cycle
- **WHEN** `Reconcile()` is called on a healthy worker
- **THEN** it SHALL execute: get session → reconcile settings → reconcile content → trigger gravity (if needed) → record metrics

#### Scenario: Settings change triggers FTL restart
- **WHEN** settings reconciliation detects changes that cause FTL restart
- **THEN** the worker SHALL wait for FTL ready, refresh the session, and proceed with content reconciliation using the fresh session

#### Scenario: Session acquisition fails
- **WHEN** the session manager returns an error
- **THEN** the worker SHALL skip all reconciliation steps, increment consecutive failures, and mark target as degraded
