...
Boolean settings SHALL be sent to the Pi-hole API as integers (`0` for `false`, `1` for `true`). The YAML config format SHALL continue to accept `true`/`false` values.

#### Scenario: Map Forseti boolean setting to API integer
- **WHEN** the effective settings contain `dns.cache.force_on_disk: true`
- **THEN** the system SHALL send the value `1` (integer) to Pi-hole config API path `dns/cache/optimizer`

#### Scenario: Map Forseti boolean false to API integer zero
- **WHEN** the effective settings contain `dns.dnssec: false`
- **THEN** the system SHALL send the value `0` (integer) to Pi-hole config API path `dns/dnssec`
...
- **WHEN** desired `dns.cache.force_on_disk` is `true` but actual is `0`, and all other settings match
- **THEN** the system SHALL write only `dns/cache/optimizer = 1` via the config API
...
The system SHALL compare desired and actual settings values using type-aware normalization. Boolean values (`true`/`false`) SHALL be treated as equivalent to their integer counterparts (`1`/`0`). Floating-point numbers without fractional parts SHALL be treated as equivalent to their integer counterparts.

#### Scenario: Bool and integer comparison
- **WHEN** the Pi-hole API returns `0` (integer, unmarshalled as `float64`) for `dns/cache/optimizer` and the desired value is `0` (coerced from `false`)
