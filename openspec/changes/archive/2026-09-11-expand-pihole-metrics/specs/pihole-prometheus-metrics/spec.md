...
- `pihole_clients_total{target}` — cumulative unique clients ever seen

#### Scenario: Client metrics populated
- **WHEN** UpdateStats is called with Stats containing clients active=15, total=42
- **THEN** `pihole_clients_active{target}` SHALL be 15 and `pihole_clients_total{target}` SHALL be 42

### Requirement: Query type breakdown metrics
The metrics server SHALL register `pihole_query_types{target, query_type}` as a GaugeVec and populate one gauge per DNS record type (A, AAAA, PTR, etc.) from the Stats query types map.
...
The metrics server SHALL register `pihole_query_status{target, status}` as a GaugeVec and populate one gauge per resolution status (GRAVITY, FORWARDED, CACHE, REGEX, etc.).
...
The metrics server SHALL register `pihole_reply_types{target, reply_type}` as a GaugeVec and populate one gauge per reply type (CNAME, IP, NXDOMAIN, etc.).
...
- `pihole_upstream_queries_total{target, upstream, name, port}` — queries routed to each upstream
...
### Requirement: Dead metrics removal
The metrics server SHALL NOT register the following metrics that lack a data source in the Pi-hole v6 API:

- `pihole_gravity_size` (redundant with `pihole_domains_blocked`)
- `pihole_cache_size`
- `pihole_cache_inserted`
- `pihole_cache_evicted`
- `pihole_ftl_memory_bytes`

#### Scenario: Removed metrics absent
- **WHEN** the metrics endpoint is scraped
- **THEN** none of the removed metric names SHALL appear in the output
